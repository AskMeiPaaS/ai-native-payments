...
| LLM | Ollama + Qwen 2.5 3B (local) | Real-time token streaming; configurable via `LLM_MODEL_NAME` |
...
│   │   ├── AccountController.java            # /api/v1/account/dashboard, /topup
...
│       └── components/
│           ├── AgentChatDashboard.tsx        # Streaming chat + channel badge
│           ├── UserBalanceDashboard.tsx      # Account summary + recent transactions
...
| `GET` | `/api/v1/account/dashboard?userId=` | Balance + recent transactions |
...
| `LLM_MODEL_NAME` | Ollama model name (default: `qwen2.5:3b`) |
